...
export function VerdictReveal({ counts, total, winner, confidence, caseTitle, accused, sentence, bestEvidence, onDone }: Props) {
  ...
  // 0: dark, 1: "jury reached a verdict", 2: bars, 3: stamp, 4: details
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-border bg-[hsl(220_30%_4%)] p-6 sm:p-10 min-h-[520px] flex flex-col grain">
      <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(ellipse_at_top,_hsl(0_84%_30%/0.25),_transparent_60%)]" />

      <div className="relative z-10 flex-1 flex flex-col">
        <p className="text-center text-xs uppercase tracking-[0.3em] text-muted-foreground">Case</p>
        <h2 className="text-center font-display text-xl sm:text-2xl mt-1 text-balance">{caseTitle}</h2>
        <p className="text-center text-sm text-muted-foreground mt-1">Accused: <span className="text-foreground font-medium">{accused}</span></p>

        <div className="flex-1 flex flex-col items-center justify-center mt-6 gap-6">
          {step >= 1 && step < 3 && (
            <p className="font-display text-2xl sm:text-3xl text-center text-foreground/90 animate-rise">
          ...
          {step >= 3 && (
            <div className="flex flex-col items-center gap-5 animate-rise">
              <div className="stamp font-stamp text-3xl sm:text-5xl animate-stamp">
                {VOTE_LABEL[winner]}
              </div>
              <p className="text-sm text-muted-foreground">Jury confidence: <span className="text-accent font-semibold">{confidence}%</span></p>
            </div>
          )}
          ...
          {step >= 4 && (
            <div className="w-full max-w-md space-y-3 animate-rise">
              <div className="court-card p-4">
                <p className="text-xs uppercase tracking-wider text-muted-foreground">Sentence</p>
                <p className="font-display text-lg mt-1 text-balance">{sentence}</p>
              </div>
              {bestEvidence && (
                <div className="court-card p-4">
                  <p className="text-xs uppercase tracking-wider text-muted-foreground">Best evidence</p>
